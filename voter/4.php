...
$sql1 = "SELECT count(Institute) From voter_master INNER JOIN voting on voter_master.Id=voting.V_id where Institute='CMPICA'";
	$q = mysqli_query($con, $sql1);
	if(mysqli_num_rows($q) > 0){
        while($row = mysqli_fetch_assoc($q)){
			$cmpica=$row["count(Institute)"];
        }
    } 
$sql2 = "SELECT count(Institute) From voter_master INNER JOIN voting on voter_master.Id=voting.V_id where Institute='ARIP'";
	$q2 = mysqli_query($con, $sql2);
	if(mysqli_num_rows($q2) > 0){
        while($row = mysqli_fetch_assoc($q2)){
			$arip=$row["count(Institute)"];
        }
    } 
$sql3 = "SELECT count(Institute) From voter_master INNER JOIN voting on voter_master.Id=voting.V_id where Institute='PDPIS'";
	$q3 = mysqli_query($con, $sql3);
	if(mysqli_num_rows($q3) > 0){
        while($row = mysqli_fetch_assoc($q3)){
			$pdpis=$row["count(Institute)"];
        }
    }  
$sql4 = "SELECT count(Institute) From voter_master INNER JOIN voting on voter_master.Id=voting.V_id where Institute='RPCP'";
	$q4 = mysqli_query($con, $sql4);
	if(mysqli_num_rows($q4) > 0){
        while($row = mysqli_fetch_assoc($q4)){
			$rpcp=$row["count(Institute)"];
        }
    } 
 $rating_data1 = array(
 array('Type', 'Rating'),
 array('CMPICA',(int)$cmpica),
 array('ARIP',(int)$arip),
 array('PDPIS',(int)$pdpis),
 array('RPCP',(int)$rpcp),
);
...
function drawChart() 
{
 ...
 var options = {
  title: "Institute Wise voted report."
 };
